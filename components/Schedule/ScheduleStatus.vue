<template>
  <div id="scheduleBox">
    ...
    <div class="list_title">ステータス</div>
    <span v-on:click="modal()" class="add_button_box">
      <img src="./assets/img/add.png" class="add_button">追加
    </span>
    ...
    <table class="list_table_min">
      <tr v-for="item, index in disp_list" :key="index">
        ...
        <td class="del_box" v-on:click="del(item.schedule_status_id)"><img src="./assets/img/del_gray.png" class="list_del"></td>
      </tr>
    </table>
  </div>
...
module.exports = {
  ...
  methods: {
    ...
    modal: function () {
      this.add_schedule_status_id = ""
      this.add_name = ""
      $('.js-modal').addClass('is-active');
    },
    modal2: function (item) {
      this.add_schedule_status_id = item.schedule_status_id
    ...
    add: function () {
      ...
      let params = new URLSearchParams();
      if (this.add_schedule_status_id) {
        params.append('type', 'update');
        params.append('schedule_status_id', this.add_schedule_status_id);
      } else {
    ...
    del(id) {
      ...
      params.append('schedule_status_id', id);
      axios.post("./php/db_schedule_status.php", params)
        .then(response => {
          this.getItem()
        })
